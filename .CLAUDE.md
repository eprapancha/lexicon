...
## 📋 Project Tracking - GitHub First!

**⚠️ IMPORTANT: GitHub is the primary source of truth for all project tracking.**
...
1. ✅ Check GitHub issues FIRST - don't rely on this file for status
2. ✅ Create/update GitHub issues for significant work
3. ✅ Reference issue numbers in commits (e.g., `fix(core): resolve bug (#42)`)
4. ✅ Update issue comments with progress, don't just update this file
5. ✅ Close issues when complete, add final summary comment
...
## 🏷️ GitHub Labels - Standardized Schema

**⚠️ CRITICAL:** This is the ONLY source of truth for labels. Before creating ANY new label, it MUST be documented here first.

### Label Categories

#### 1. Priority (Mutually Exclusive - Pick ONE)
- `priority-critical` - 🔴 **Red** - Blocking, must fix immediately (build broken, data loss)
- `priority-high` - 🟠 **Orange** - Important, fix this sprint (regressions, architectural flaws)
- `priority-medium` - 🟡 **Yellow** - Normal priority, fix when able (features, enhancements)
- `priority-low` - 🟢 **Green** - Nice to have, fix when time permits (minor improvements)

**Rules:**
- Every issue MUST have exactly ONE priority label
- Default to `priority-medium` if unsure
- Only maintainer can assign `priority-critical`

#### 2. Type (Mutually Exclusive - Pick ONE)
- `type-bug` - 🐛 **Red** - Something is broken (regressions, failures, crashes)
- `type-feature` - ✨ **Blue** - New functionality (commands, modes, UI)
- `type-refactor` - 🔧 **Purple** - Code improvement, no behavior change
- `type-test` - 🧪 **Cyan** - Test-related (add tests, fix tests, test infrastructure)
- `type-docs` - 📚 **Green** - Documentation only (README, guides, comments)
- `type-deps` - 📦 **Gray** - Dependency updates (npm, cargo, build tools)
- `type-architecture` - 🏗️ **Gold** - Structural changes (state model, ownership, design)

**Rules:**
- Every issue SHOULD have ONE type label (optional for epics)
- Use `type-architecture` for design-level issues (like Issue #65)

#### 3. Phase (Multiple Allowed)
- `phase-7` - Phase 7.x work (minibuffer, completion, current phase)
- `phase-8` - Phase 8.x work (elisp compatibility, future)
- `phase-9` - Phase 9.x work (LSP, syntax, future)

**Rules:**
- Phase labels track WHEN work is planned
- Multiple phases OK if issue spans phases
- Remove phase label when complete

#### 4. Component (Multiple Allowed)
- `component-buffer` - 📝 Buffer management (text storage, WASM, gap buffer)
- `component-window` - 🪟 Window/frame management (splits, layout, focus)
- `component-keymap` - ⌨️ Keybindings (command dispatch, key sequences)
- `component-minibuffer` - 💬 Minibuffer (completion, input, echo area)
- `component-mode` - 🎨 Major/minor modes (mode system, mode-line)
- `component-edit` - ✏️ Editing commands (insert, delete, kill/yank, region)
- `component-file` - 💾 File I/O (open, save, auto-save)
- `component-test` - 🧪 Test infrastructure (E2E, unit tests, test helpers)
- `component-wasm` - ⚙️ Rust/WASM layer (gap buffer, bindings)
- `component-state` - 🗄️ State management (re-frame, db, ownership)
- `component-ui` - 🖼️ UI rendering (React, views, CSS)
- `component-hooks` - 🪝 Hook system (pre/post command, file hooks)

**Rules:**
- Multiple components OK if issue spans areas
- Use most specific component possible
- Add new components sparingly (requires discussion)

#### 5. Status (Mutually Exclusive - Pick ONE)
- `status-blocked` - 🚫 **Red** - Cannot proceed (waiting on dependency, decision)
- `status-in-progress` - 🔄 **Blue** - Actively being worked on
- `status-review` - 👀 **Purple** - PR open, awaiting review
- `status-stale` - 💤 **Gray** - No activity for >30 days, may close

**Rules:**
- Status labels are OPTIONAL (absence = "not started")
- Remove `status-in-progress` when issue complete
- Use `status-blocked` with comment explaining blocker

#### 6. Special Labels
- `good-first-issue` - 🌱 **Green** - Suitable for new contributors
- `help-wanted` - 🙋 **Purple** - Looking for community help
- `breaking-change` - ⚠️ **Red** - Will break existing functionality
- `regression` - 🔙 **Orange** - Previously working, now broken
- `security` - 🔒 **Red** - Security implications (XSS, eval, sandboxing)
- `performance` - ⚡ **Yellow** - Performance-related (slow, optimization)
- `accessibility` - ♿ **Blue** - A11y improvements

**Rules:**
- Special labels supplement other categories
- Use `regression` for issues caused by recent changes
- Use `breaking-change` for PRs that change public API

### Label Rules

1. **NEVER create labels ad-hoc** - All labels MUST be documented here first
2. **NEVER duplicate labels** - Check existing labels before creating (e.g., no `medium-priority` AND `priority-medium`)
3. **Naming convention:** `category-value` (kebab-case)
   - ✅ `priority-high`, `type-bug`, `component-buffer`
   - ❌ `high-priority`, `Bug`, `buffer_component`
4. **Color coding:** Use colors consistently per category (see above)
5. **Required labels:** Every issue MUST have at least:
   - ONE priority label (`priority-*`)
   - ONE type label (`type-*`) OR be an epic
6. **Updating labels:** If you need a new label:
   1. Update this section FIRST
   2. Get approval from maintainer
   3. Then create in GitHub
   4. Document why it was needed

### Label Cleanup (2026-01-11)

**Issues Found:**
- ❌ Inconsistent naming: `medium-priority` vs `priority-medium`
- ❌ Missing categories (no type labels on some issues)
- ❌ Unlabeled issues (no priority assigned)

**Action Items:**
- [ ] Audit all existing labels in GitHub
- [ ] Delete duplicates (keep `priority-medium`, delete `medium-priority`)
- [ ] Standardize to schema above
- [ ] Add missing required labels to all issues
- [ ] Document label changes in issue comments

### Creating New Labels

If you determine a new label is needed:
...
2. **Justify why it's needed:**
   - What existing label doesn't cover this?
   - How many issues would use this label?
...
4. **Create in GitHub:**
   ```bash
   gh label create "category-name" --description "Description" --color "HexCode"
   ```

5. **Apply to relevant issues:**
   ```bash
   gh issue edit <issue-number> --add-label "category-name"
   ```

### Example: Labeling an Issue
...
gh issue edit 65 \
  ...
  --add-label "type-architecture" \
  --add-label "component-state" \
  --add-label "regression" \
  --add-label "phase-7"
```

**Rationale:**
- `priority-critical` - Blocking E2E tests, causes regressions
- `type-architecture` - State model flaw, not simple bug
- `component-state` - Cursor state ownership issue
- `regression` - Caused by commit 9358d79
- `phase-7` - Current phase work
...
- ✅ **Phase 7.8.1:** Minibuffer Architecture Redesign ([#41-55](https://github.com/eprapancha/lexicon/issues?q=is%3Aissue+label%3Aphase-7.8+is%3Aclosed))
...
- 🔄 **Issue #60:** Package Isolation + State Ownership Violations
  - ✅ Phase 1: Fixed 3 minibuffer/echo-area violations
  - ✅ Phase 2: Created `:window/set-mark`, `:window/set-cursor` events, fixed 7 violations
  - ✅ Phase 3: Fixed 1 buffer metadata violation
  - ✅ Phase 4: Documented state ownership rules in db.cljs
  - 📊 **Progress:** 11 violations fixed, build passing with 0 warnings
...
**State Ownership Map:**
- `:mark-position` → OWNED by `edit.cljs` (mark/region commands)
  - Use events: `:set-mark`, `:deactivate-mark`, `:exchange-point-and-mark`
  - ❌ NEVER: `(assoc % :mark-position nil)` from query-replace
  - ✅ ALWAYS: `[:dispatch [:deactivate-mark]]` from anywhere

- `:minibuffer` → OWNED by `ui.cljs`
  - Use events: `:minibuffer/activate`, `:minibuffer/deactivate`, `:minibuffer/set-input`
  - ❌ NEVER: `(assoc db :minibuffer {...})` from isearch
  - ✅ ALWAYS: `[:dispatch [:minibuffer/activate {...}]]`

- `:echo-area` → OWNED by `ui.cljs`
  - Use events: `:echo/message`, `:echo/clear`
  - ❌ NEVER: `(assoc-in db [:echo-area :message] ...)` from isearch
  - ✅ ALWAYS: `[:dispatch [:echo/message "..."]]`

- `:window-tree` → OWNED by `ui.cljs` (window management)
  - Use events: `:window/set-buffer`, `:split-window-below`, `:delete-window`
  - ❌ NEVER: `(update :window-tree db/update-window-in-tree ...)` from edit.cljs
  - ✅ ALWAYS: Dispatch events or extract to window management

- `:ui :cursor-position` → OWNED by `edit.cljs` (or dedicated cursor module)
  - Use event: `:cursor/set-position` (to be created)
  - ❌ NEVER: `(assoc-in db [:ui :cursor-position] ...)` from isearch/buffer
  - ✅ ALWAYS: `[:dispatch [:cursor/set-position pos]]`

- `:buffers` → OWNED by `buffer.cljs`
  - Use events for buffer metadata: `:buffer/set-mode`, `:buffer/update-version`, etc.
  - ❌ NEVER: `(assoc-in db [:buffers id :major-mode] ...)` from mode.cljs
  - ✅ ALWAYS: `[:dispatch [:buffer/set-mode buffer-id mode]]`

- `:kill-ring` → OWNED by `edit.cljs` (kill/yank commands)
...
**✅ Fixed Violations (Issue #60 - 2026-01-11):**
...
- ✅ **Rust code:** Unit tests required for all public functions
- ✅ **E2E tests:** All user-facing features must have E2E tests
- ✅ **Test-first when possible:** Write tests before implementation for new features
...
**File Size Limits:**
- ✅ **Max ~500 lines per namespace** - Split larger files into focused modules
- ✅ **Example:** `events.cljs` refactored from 2,958 lines → 7 modules (~400 lines each)
...
├── hooks.cljs             # Hook system
├── markers.cljs           # Marker abstraction
├── context.cljs           # Dynamic execution context
└── advanced_undo.cljs     # Undo system
```
...
**⚠️ NEVER Run Compilation or Dev Servers:**
- ❌ NEVER run `bb dev` or any dev server
- ❌ NEVER run `npx shadow-cljs compile/watch`
- ❌ NEVER run `cargo build` or `wasm-pack build`
- ❌ NEVER run background compilation processes
- ❌ NEVER run tests in background where output is hidden
...
- ✅ If you don't see compilation output, don't assume it worked
- ✅ User will report compilation errors when they occur
- ✅ Ask before adding new learnings to project memory

### Commit Hygiene

**Commit Frequency:**
- ✅ **Commit after each green test run** - Keep commits small and atomic
- ✅ **Commit after completing each step** in roadmap tasks
- ✅ **Commit format:**
  ```
...
- Use `print-step` for progress indication:
  ```clojure
  (print-step "🚀" "Starting build...")
  (print-step "✅" "Build complete!")
  (print-step "❌" "Build failed!")
  ```
...
- ✅ Run utility libraries (dash, s)
- ✅ Load orderless, partial consult/embark
...
(js/console.warn "⚠️ Package load slow:" package-name time-ms)
(js/console.error "❌ Hook failed:" hook-id error)
```
...
1. ✅ **Test first** - Write tests, then implement
2. ✅ **Commit frequently** - After each green test run
3. ✅ **No warnings** - Fix all warnings before commit
4. ✅ **Small functions** - Refactor when file > 500 lines
5. ✅ **Emacs fidelity** - When in doubt, check Emacs source
6. ✅ **Document as you go** - Update ROADMAP.md when completing phases
7. ✅ **Zero regressions** - All tests must pass
8. ✅ **Git for debugging** - Use `git diff` and `git bisect` to find regressions
