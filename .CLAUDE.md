...
**Status:** Phase 6.5 Planning Complete - Ready to Begin Implementation
...
**Why Phase 6.5?** Emacs core comparison analysis revealed critical foundational gaps that will cause Phase 8 (Completion Ecosystem) to fail. We need these foundations FIRST:

1. **Prefix Arguments (C-u)** - Commands need behavior modifiers
2. **Minibuffer State Machine** - Stack-based, recursive, proper command handoff
3. **Buffer-Local Environments** - Modes need per-buffer execution contexts
4. **Runtime Evaluation (SCI)** - User init files, interactive eval, extensibility

### Issues Created

**Epic:** [#75](https://github.com/eprapancha/lexicon/issues/75) - Phase 6.5: Lexicon Core Foundation (8 weeks)

**Sub-Issues:**
- [#76](https://github.com/eprapancha/lexicon/issues/76) - Week 1-2: Prefix Arguments (C-u System) - **priority-high**
- [#77](https://github.com/eprapancha/lexicon/issues/77) - Week 3-4: Minibuffer State Machine - **priority-critical** (Fixes [#72](https://github.com/eprapancha/lexicon/issues/72))
- [#78](https://github.com/eprapancha/lexicon/issues/78) - Week 5-6: Buffer-Local Environments - **priority-high**
- [#79](https://github.com/eprapancha/lexicon/issues/79) - Week 7-8: Runtime Evaluation (SCI Integration) - **priority-medium**

**Documentation:**
- `docs/phase-6.5-research.md` - Comprehensive Emacs source research summary
- Research sources: Emacs source in `/tmp/emacs-source`, SCI documentation

**Next Steps:** Begin implementation with #76 (Prefix Arguments)

### Recent Completed Work

- ✅ **Issue #71** - kill-word (M-d) regression fixed
- ✅ **Issue #73** - Log Bus implemented, *Messages* buffer lifecycle-independent
- ✅ **Core API Re-exports** - `lexicon.core` now provides unified API (message, point, goto-char, etc.)
- ✅ **Phase 6.5 Planning** - Complete research, 5 issues created with detailed specs
...
**⚠️ CRITICAL:** This is the ONLY source of truth for labels.

### Required Labels (Every Issue)

**Priority (Pick ONE):**
- `priority-critical` 🔴 - Blocking (build broken, data loss)
- `priority-high` 🟠 - Important (regressions, architectural)
- `priority-medium` 🟡 - Normal (features, enhancements)
- `priority-low` 🟢 - Nice to have

**Type (Pick ONE, or epic):**
...
**Phase:** `phase-6.5`, `phase-7`, `phase-8` (when work is planned)

**Component:** `component-minibuffer`, `component-commands`, `component-buffers`, etc.

**Status:** `status-blocked`, `status-in-progress` (optional, absence = "not started")

**Special:** `epic` (large multi-issue feature), `e2e-failure`, `regression`

### Label Creation Rules

1. **NEVER create labels ad-hoc**
2. Update this file FIRST before creating in GitHub
3. Naming: `category-value` (kebab-case)
4. Get maintainer approval for new labels
5. Document why needed
