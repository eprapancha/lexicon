...
(defmacro save-excursion
  ...
  [& body]
  `(let [saved-point# (lexicon.api.test/point (lexicon.api.test/current-buffer))
         saved-mark# (lexicon.api.test/mark (lexicon.api.test/current-buffer))
         result# (atom nil)]
     (try
       ...
       (finally
         ...
         (when saved-point#
           (lexicon.api.test/set-point! (lexicon.api.test/current-buffer) saved-point#))
         (when saved-mark#
           (lexicon.api.test/set-mark! (lexicon.api.test/current-buffer) saved-mark#))))
     @result#))
...
(defmacro save-current-buffer
  ...
  [& body]
  `(let [saved-buffer-id# (lexicon.api.test/current-buffer)
         result# (atom nil)]
     (try
       (reset! result# (do ~@body))
       (finally
         ...
         (when saved-buffer-id#
           (lexicon.api.test/show-buffer! saved-buffer-id#))))
     @result#))
...
(defmacro with-current-buffer
  ...
  [buffer-id & body]
  `(save-current-buffer
     (lexicon.api.test/show-buffer! ~buffer-id)
     ~@body))
