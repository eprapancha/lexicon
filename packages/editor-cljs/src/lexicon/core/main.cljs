(ns lexicon.core.main
  (:require [re-frame.core :as rf]
            ...
            [lexicon.packages.kmacro]          ; Load keyboard macros (#121)
            [lexicon.core.modes.rectangle]     ; Load rectangle operations (#121)
            [lexicon.packages.dabbrev]         ; Load dabbrev M-/ expansion (#120)
            ...
            [lexicon.packages.occur]           ; Load occur-mode (#131)
            ...
            [lexicon.packages.hi-lock :as hi-lock]     ; Load hi-lock highlighting (#125)
            [lexicon.packages.eldoc :as eldoc]      ; Load eldoc documentation (#124)
            [lexicon.packages.ibuffer :as ibuffer]  ; Load ibuffer (#140)
            [lexicon.core.modes.diff-mode :as diff-mode] ; Load diff-mode (#119)
            [lexicon.packages.vc :as vc]               ; Load vc.el version control (#113)
            [lexicon.packages.xref :as xref]        ; Load xref.el cross-references (#116)
            [lexicon.packages.shell :as shell]         ; Load shell/eshell (#112)
            [lexicon.packages.info :as info]           ; Load info.el documentation browser (#142)
            [lexicon.packages.eglot :as eglot]          ; Load eglot LSP client (#129)
            [lexicon.packages.grep :as grep]               ; Load grep.el search (#125)
            [lexicon.packages.ediff :as ediff]         ; Load ediff/smerge-mode (#119)
...
(defn init
  ...
  ;; Initialize tab-bar-mode (#152)
  (tab-bar/init-tab-bar!)

  ;; Initialize hi-lock highlighting (#125)
  (hi-lock/init-hi-lock!)
