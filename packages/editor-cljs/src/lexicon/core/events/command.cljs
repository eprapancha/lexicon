(ns lexicon.core.events.command
...
(rf/reg-event-fx
 ...
 (fn [{:keys [db]} [_ command-name]]
   ...
   (log/info (str "execute-command-interactive: " command-name))
   (let [command-def (get-in db [:commands command-name])
         interactive-spec (:interactive command-def)]
     (log/info (str "interactive-spec: " (pr-str interactive-spec)))
     (if interactive-spec
       ...
       (let [collected (interactive/collect-all-args db interactive-spec)
         ...
         (if (empty? async-prompts)
           ...
           ;; Use :replace? true to take over the M-x minibuffer slot
...
(rf/reg-event-fx
 ...
 (fn [{:keys [db]} [_ user-input]]
   ...
   (log/info (str "collect-next-arg: " user-input))
   (let [pending (get-in db [:command-execution :pending])
         _ (log/info (str "pending: " (pr-str pending)))
