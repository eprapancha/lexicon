...
(defn get-cursor-linear-position
  "Get current cursor position as linear offset from buffer start"
  []
  (let [script "
    const state = window.editorState;
    if (!state) return 0;
...
(defn send-emacs-key
  "Send an Emacs-style key sequence (e.g., 'C-s', 'C-r', 'M-x', 'DEL', 'RET')"
  [key-str]
  (cond
    ;; Control keys
    (.startsWith key-str "C-")
    (let [key (subs key-str 2)]
      (press-ctrl-key key))

    ;; Meta/Alt keys
    (.startsWith key-str "M-")
    (let [key (subs key-str 2)]
      (press-meta-key key))

    ;; Special keys
    (= key-str "RET") (press-key "Enter")
    (= key-str "DEL") (press-key "Backspace")
    (= key-str "SPC") (type-text " ")
    (= key-str "TAB") (press-key "Tab")

    ;; Single character
    :else (type-text key-str)))
...
(deftest test-p7-8-07-isearch-forward-basic
  (testing "P7.8-07: Basic incremental search forward (C-s)"
    ...
    (let [cursor-pos (get-cursor-linear-position)]
      (is (= 16 cursor-pos)
...
(deftest test-p7-8-08-isearch-repeat-forward
  (testing "P7.8-08: Repeat search forward (C-s during isearch)"
    ...
    (type-text "The quick brown fox")
    (press-key "Enter")
    (type-text "jumps over the lazy dog")
    (press-key "Enter")
    (type-text "The fox is quick")
    (Thread/sleep 50)
...
(deftest test-p7-8-11-isearch-abort
  (testing "P7.8-11: Abort isearch with C-g (return to original position)"
    ...
    ;; Insert test text
    (type-text "The quick brown fox jumps over the lazy dog")
    (Thread/sleep 50)
    ...
    (let [original-pos (get-cursor-linear-position)]
      (is (= 20 original-pos)
      ...
      (let [search-pos (get-cursor-linear-position)]
        (is (= 40 search-pos)
      ...
      (let [final-pos (get-cursor-linear-position)]
        (is (= 20 final-pos)
...
(deftest test-p7-8-13-isearch-wrap-around
  (testing "P7.8-13: Search wraps around at end of buffer"
    ...
    ;; Insert test text
    (type-text "first line has word")
    (press-key "Enter")
    (type-text "second line")
    (press-key "Enter")
    (type-text "third line has word")
    (Thread/sleep 50)
